...
kos.onkeyup = function () {
  if (this.value) {
    ...
    s.src =
      "https://s.video.qq.com/smartbox?callback=fn&plat=2&ver=0&num=5&otype=json&query=" +
      ...
      success: function (res) {
        if (res.status == 200) {
          ...
          $.each(res, function (index, value) {
            lihtml =
              lihtml +
              '<li class="wei"><a class="a" href=javascript:play(' +
              value["url"] +
              ',"' +
              value["title"] +
              '")>' +
    ...
    success: function (res) {
      if (res.status == 200) {
        ...
        $.each(res, function (index, value) {
          lihtml =
            lihtml +
            '<li class="wei"><a class="a" target="_blank" href="' +
            value["url"] +
            '">' +
            value["title"] +
            "</a></li>";
...
function fn(data) {
  ...
  data.item.forEach(function (item, index) {
    lihtml =
      lihtml +
      '<div class="wei" cursor="pointer"><li class="a">' +
      item["word"] +
      "</li></div>";
  });
  $("ul[id='kol']").html(lihtml);
